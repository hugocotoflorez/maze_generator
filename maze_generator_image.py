...
def decision_tree(index,maze):
    ...
    newindex1 = lambda x2,y2: (y+y2) * (size[0]*2-1) + (x+x2)
    newindex = lambda x2,y2: (y+2*y2) * (size[0]*2-1) + (x+2*x2)
...
with Image.open(PATH) as im:  
    ...
    to2d: (tuple[int,int]) = lambda index:(index//(size[0]*2-1), index%(size[0]*2-1))
    ...
    for i in range(1,100):
        ...
        try:
            ...
            decision_tree(index = (size[0]*2-1) * (size[1]*2-3) +1, maze = maze)
            print('Done')
        ...
        try:
            ...
            im1.crop((0,0,a*(size[0]*2-1), a*(size[1]*2-1))).save('maze.jpg')
            print('Done')
        ...
        try:
            ...
            maze2d = [[1 if e == WALL else 0 for e in maze[(size[0]*2-1)*i:(size[0]*2-1)*(i+1)]] for i in range(size[1]*2-1)]
            solution = astar.aStarSearch(maze2d,to2d(1), to2d((size[0]*2-1)*(size[1]*2-1)-2))
            print('Done')
        ...
        try:
            ...
            solvedim.crop((0,0,a*(size[0]*2-1), a*(size[1]*2-1))).save('maze_solved.jpg')
            print('Done')
            break
